...
#define MSG_CHANNEL_MAX (65536 - 20 - 8)    // 20: ip package head, 8:udp package
...
// 每一個频道内容结构体：频道号, data(指针)
struct msg_channel_st
{
    /* data */
    chnid_t chnid;  /*must between [MINCHNID, MAXCHIND]*/
    uint8_t data[1];
...
struct msg_listentry_st
{
    /* data */
    chnid_t chnid;
    uint16_t len;   // 频道的描述信息例如：pop music, classical music...
    uint8_t desc[1];
}__attribute__((packed));   // do not align
